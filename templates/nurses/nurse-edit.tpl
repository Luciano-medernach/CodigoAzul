...
<body class=" bg-gradient-to-b from-cwhite to-clightblue min-h-screen ">
    <form class="mt-32 flex flex-col  items-center text-lg h-64 w-1/2 mx-auto box-with-shadow p-4 "  action="db-nurse-add" method="POST">
        ...
        <input class="bg-cgray/50 text-cdarkgray inputs-form" type="text" name="name" placeholder="Nombre" maxlength="50" value="{$nurse->name}" required />
        <input class="mt-2 bg-cgray/50 text-cdarkgray inputs-form mb-8"  type="text" name="lastname" placeholder="Apellido" maxlength="50" value="{$nurse->lastname}" required />
